...
Returns a canonical CPMProblem for Steinberg differential adhesion hypothesis cell sorting.
"""
function cell_sorting_problem(;
    ...
    return CPMProblem(sys, Dict(A => cells_per_type, B => cells_per_type), grid_size; tspan=tspan, topology=topology)
end

"""
    young_laplace_droplet(; kwargs...)

Returns a canonical CPMProblem for verifying the Young-Laplace equation for a single droplet.
"""
function young_laplace_droplet(;
    ...
    # The default CPMProblem initialization spawns cells randomly, but for this specific test
    # we usually want it placed precisely in the center to avoid boundary effects. 
    prob = CPMProblem(sys, Dict(Cell => 1), grid_size; tspan=tspan, topology=topology)
    ...
    CoreCPM.spawn_hypersphere!(prob.u0.grid, grid_size, center, radius, UInt32(2))
    
    # Sync volume tracker. CPMToolkit maps types to IDs starting from 1 for non-medium.
...
Returns a canonical CPMProblem for measuring thermodynamic volume fluctuations.
"""
...
Returns a canonical CPMProblem for testing 2D Ising Model critical phase transitions.
"""
function ising_model(;
    ...
    prob = CPMProblem(sys, Dict(Up => 1, Down => 1), grid_size; tspan=tspan, topology=topology, max_cells=3)
