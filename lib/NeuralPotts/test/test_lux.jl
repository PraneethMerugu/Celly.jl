...
state = PottsState(grid, cell_data)
...
function mcmc_callback(state, loss_val)
    ...
    for i in 1:length(train_cache.persistent_states)
        CorePotts.execute_step!(train_cache.persistent_states[i], p_new, train_cache.caches[i], train_cache.alg)
    end
...
optf = OptimizationFunction(
    (θ, p) -> potts_loss(θ, p.train_cache, p.data_batch, p.update_fn),
    AutoZygote()
)
opt_prob = OptimizationProblem(optf, theta, p_context)

println("Training Neural Potts...")
