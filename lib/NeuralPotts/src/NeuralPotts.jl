...
using CorePotts
using CorePotts: PottsState, PottsParameters, LocalNeuralPenalty, AbstractPenalty
...
function CorePotts.evaluate_penalty(p::LocalNeuralPenalty, ctx)
...
function CorePotts.compute_global_energy(p::LocalNeuralPenalty, u::PottsState, params::PottsParameters)
...
# ------------------------------------------------------------------------------
# The user can define how their ComponentArray `theta` maps to the `PottsParameters`.
# For a LocalNeuralPenalty, they might map `theta` directly to `weights`.

function CorePotts.potts_loss(theta, cache::CorePotts.PottsTrainingCache, data_batch)
    error("potts_loss must be differentiated with an AD backend. Make sure Zygote or Enzyme is loaded, and ignore_derivatives is used for the MCMC step.")
end
