...
function step!(compInt::_CPMComponentIntegrator)
    # The CPM Integrator intrinsically handles its own time mapping, but Mermaid requests a step.
...
function Mermaid.getstate(compInt::_CPMComponentIntegrator)
...
function Mermaid.setstate!(compInt::_CPMComponentIntegrator, key::ConnectedVariable, value)
    ...
    if first(var) == '#'
        ...
        elseif var == "#state"
            ...
            return nothing
        elseif var == "#ids"
            @warn "Cannot explicitly set #ids for CoreCPM. Cells are spawned via mitosis/death."
            return nothing
        end
    end
