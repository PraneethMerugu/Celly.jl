...
function CoreCPM.initialize_backend(backend::CoreCPM.ZarrBackend, prob, alg, opts)
    ...
    zarr_grid = Zarr.zcreate(eltype(grid), zgroup, "grid", grid_size..., num_saves; chunks=chunk_size, fill_value=zero(eltype(grid)))
    ...
    for field in propertynames(u0.cell_data)
        ...
        zarr_array = Zarr.zcreate(T_el, cell_data_group_zarr, string(field), max_capacity, num_saves; chunks=(max_capacity, 1), fill_value=zero(T_el))
