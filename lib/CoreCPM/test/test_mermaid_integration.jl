...
@testset "Mermaid Integration" begin
    # 1. Setup a simple CPM simulation
    W, H = 20, 20
    N_cells = 3
    ...
    penalties = (VolumePenalty{Rigid}(Float32[1.0, 1.0, 1.0]), VolumePenalty{Rigid}(Float32[2.0, 2.0, 2.0]), AdhesionPenalty(ones(Float32, 3, 3)))
    trackers = (VolumeTracker(), SurfaceAreaTracker())
    
    u0 = CPMState(grid, cell_data, N_cells)
    p = CPMParameters(MooreTopology{2}(), penalties, trackers)
    
    alg = SequentialMetropolis(; T=1.0f0)
    prob = CPMProblem(u0, (0.0, 10.0), p)
    
    # 2. Create the Mermaid component
    cpm_comp = CPMComponent(prob, alg; name="Tissue", timestep=1.0)
    ...
    @test Mermaid.getstate(integrator, Mermaid.ConnectedVariable("Tissue", "#time", nothing, nothing)) == 0.0
    @test Mermaid.getstate(integrator, Mermaid.ConnectedVariable("Tissue", "target_volumes", 1, nothing)) == cell_data.target_volumes[1]
    
    # Test reading the collision penalty
    @test Mermaid.getstate(integrator, Mermaid.ConnectedVariable("Tissue", "VolumePenalty[2].lambdas", 2, nothing)) == 2.0f0
    ...
    Mermaid.setstate!(integrator, Mermaid.ConnectedVariable("Tissue", "VolumePenalty[1].lambdas", [2], nothing), [2.0])
    ...
    Mermaid.setstate!(integrator, Mermaid.ConnectedVariable("Tissue", "target_volumes", nothing, nothing), [100.0, 300.0])
