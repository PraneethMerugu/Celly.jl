...
@testset "HSTLengthPenalty Initial State and SDE" begin
    ...
    cell_data = CoreCPM.build_cell_data(grid, 1)
    ...
    CoreCPM.sync_cell_data!(u0, p_sys, cache, 1)
    
    # Run auxiliary field update (this should populate current_lengths, major_axis, length_pressures)
    CoreCPM.update_step_auxiliary!(penalty, u0, p_sys, cache, 1.0f0, 0.01f0)
    ...
    ctx = (; grid=grid, grid_dims=(W, H), topology=MooreTopology{2}(), cell_data=cell_data, trackers=(), idx=UInt32(1), src=UInt32(0), tgt=UInt32(1), T=1.0f0, spatial_coords=(20, 71), neighbors=ntuple(i->0, 8), n_src=Int32(0), n_tgt=Int32(0))
...
@testset "HSTLengthPenalty Thermodynamic Elongation" begin
    ...
    problem = CPMProblem(u0, (0, 500), p_sys)
    ...
    CoreCPM.update_step_auxiliary!(len_pen, integrator.u, integrator.p, integrator.cache, 10.0f0, 0.1f0)
