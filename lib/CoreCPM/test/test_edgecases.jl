...
@testset "Edge Case and Engineering Tests" begin
    @testset "G. Strict Zero-Temperature Quench" begin
        ...
        function global_energy(u)
            ...
            for x in 1:W, y in 1:H_dim
                ...
                for (dx, dy) in CoreCPM.lottery_offsets(MooreTopology{2}())
    ...
    @testset "H. Cell Fragmentation / Connectivity" begin
        ...
        grid[5:15, 5:10] .= 1; grid[5:15, 11:15] .= 2
        
        cell_data = build_cell_data(grid, 2)
        cell_data.cell_types[1] = 1; cell_data.cell_types[2] = 1
        cell_data.target_volumes[1] = 50; cell_data.target_volumes[2] = 50
        ...
        function check_connectivity(g, c_id)
            ...
            for x in 1:W, y in 1:H
                if g[x,y] == c_id
                    start_x, start_y = x, y; break
                end
            end
            if start_x == 0 return 0 end
        ...
        CoreCPM.sync_cell_data!(integrator_boil.u, integrator_boil.p, integrator_boil.cache, 2)
