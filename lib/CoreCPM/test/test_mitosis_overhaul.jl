...
# Add CoreCPM to LOAD_PATH to allow importing
...
@testset "Cell Life-Cycle Overhaul" begin
    ...
    process_mitosis_events!(u0, p_sys, cache, ws; trigger=VolumeThresholdTrigger(1.0f0), orientation=RandomOrientation())
    ...
    process_mitosis_events!(u0, p_sys, cache, ws; trigger=VolumeThresholdTrigger(1.0f0), orientation=MajorAxisOrientation())
    ...
    CoreCPM.required_fields(::ProteinTrigger) = (:proteins, :target_volumes)
    ...
    process_mitosis_events!(u0_2, p_sys_2, cache_2, ws_2; trigger=ProteinTrigger(), inheritance_rules=rules, orientation=RandomOrientation())
