...
@kernel function _grid_accumulate_com_kernel!(grid, grid_dims, acc_sin_x, acc_cos_x, acc_sin_y, acc_cos_y, acc_sin_z, acc_cos_z, sin_lut_x, cos_lut_x, sin_lut_y, cos_lut_y, sin_lut_z, cos_lut_z)
...
@kernel function _cell_normalize_com_kernel!(anchors_x, anchors_y, anchors_z, acc_sin_x, acc_cos_x, acc_sin_y, acc_cos_y, acc_sin_z, acc_cos_z, vols, grid_dims)
    ...
    if my_cell_id <= length(vols)
        ...
        if V > zero(F)
            ...
            if length(grid_dims) == 3
                D = F(grid_dims[3])
                az = (D / (F(2.0) * F(pi))) * atan(acc_sin_z[my_cell_id], acc_cos_z[my_cell_id])
...
@kernel function _compute_forces_kernel!(anchors_x, anchors_y, forces_x, forces_y, vols, ctypes, connectivity, lambdas, target_lengths, grid_dims, topo)
    my_cell_id = @index(Global, Linear)
            ...
            for other_id in 1:size(connectivity, 2)
                if connectivity[my_cell_id, other_id] > 0
                    if vols[other_id] > 0
                        ...
                        if dist > F(0.0001)
                            mag = F(lambdas[my_type + 1]) * (dist - F(target_lengths[my_type + 1]))
                            Fx += mag * (dx / dist)
                            Fy += mag * (dy / dist)
                        end
                    end
...
function update_sweep_auxiliary!(pen::FocalPointSpringPenalty, u::AbstractCPMState, p::CPMParameters, cache::CPMCache, T_val, dt)
    ...
    k_grid(u.grid, cache.grid_dims, u.cell_data.com_acc_sin_x, u.cell_data.com_acc_cos_x, u.cell_data.com_acc_sin_y, u.cell_data.com_acc_cos_y, u.cell_data.com_acc_sin_z, u.cell_data.com_acc_cos_z, cache.sin_lut_x, cache.cos_lut_x, cache.sin_lut_y, cache.cos_lut_y, cache.sin_lut_z, cache.cos_lut_z, ndrange=length(u.grid))
    KernelAbstractions.synchronize(backend)
    
    k_cell = _cell_normalize_com_kernel!(backend, cache.block_size)
    k_cell(u.cell_data.anchor_x, u.cell_data.anchor_y, u.cell_data.anchor_z, u.cell_data.com_acc_sin_x, u.cell_data.com_acc_cos_x, u.cell_data.com_acc_sin_y, u.cell_data.com_acc_cos_y, u.cell_data.com_acc_sin_z, u.cell_data.com_acc_cos_z, u.cell_data.volumes, cache.grid_dims, ndrange=N)
    KernelAbstractions.synchronize(backend)
    
    k2 = _compute_forces_kernel!(backend, cache.block_size)
    k2(u.cell_data.anchor_x, u.cell_data.anchor_y, u.cell_data.force_x, u.cell_data.force_y, u.cell_data.volumes, u.cell_data.cell_types, pen.connectivity, pen.lambdas, pen.target_lengths, cache.grid_dims, p.topology, ndrange=N)
    KernelAbstractions.synchronize(backend)
end
...
@kernel function _compute_hst_forces_kernel!(anchors_x, anchors_y, forces_x, forces_y, vols, ctypes, connectivity, lambdas, target_lengths, eta, T_val, seed, dt, grid_dims, topo)
    my_cell_id = @index(Global, Linear)
    ...
    if my_cell_id <= length(vols)
        ...
        if V > zero(F)
            ...
            noise_x = noise_std * F(randn_pcg(seed + UInt64(my_cell_id), seed + UInt64(my_cell_id + 11111)))
            noise_y = noise_std * F(randn_pcg(seed + UInt64(my_cell_id), seed + UInt64(my_cell_id + 22222)))
            
            forces_x[my_cell_id] = alpha * forces_x[my_cell_id] + (one(F) - alpha) * mean_Fx + noise_x
            forces_y[my_cell_id] = alpha * forces_y[my_cell_id] + (one(F) - alpha) * mean_Fy + noise_y
        end
    end
end

function update_step_auxiliary!(pen::HSTFocalPointPenalty, u::AbstractCPMState, p::CPMParameters, cache::CPMCache, T_val, dt)
    ...
    k2(u.cell_data.anchor_x, u.cell_data.anchor_y, u.cell_data.force_x, u.cell_data.force_y, u.cell_data.volumes, u.cell_data.cell_types, pen.connectivity, pen.lambdas, pen.target_lengths, pen.eta, T_val, seed, dt, cache.grid_dims, p.topology, ndrange=N)
    KernelAbstractions.synchronize(backend)
end
