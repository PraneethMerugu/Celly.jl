...
"""
    SciMLBase.solve!(integrator::CPMIntegrator)

In-place execution of the CPM simulation. Steps the `integrator` forward in time until it reaches
...
function SciMLBase.init(prob::CPMProblem, alg::AbstractCPMAlgorithm, args...; kwargs...)
    return SciMLBase.__init(prob, alg, args...; kwargs...)
end

"""
    SciMLBase.solve(prob::CPMProblem, alg::AbstractCPMAlgorithm, args...; kwargs...)

Standard out-of-place execution interface. Dynamically initializes a new `CPMIntegrator`,
allocates engine structures, executes the full simulation, and returns a `CPMSolution`.
"""
function SciMLBase.solve(prob::CPMProblem, alg::AbstractCPMAlgorithm, args...; kwargs...)
