...
@inline function get_lottery_neighbor_idx(topo::AbstractTopology{N}, coords::NTuple{N, UInt32}, dir::Int, dims::NTuple{N, Int}) where N
...
@inline function get_lottery_neighbor_idx(topo::Union{NoFluxVonNeumannTopology{N}, NoFluxMooreTopology{N}}, coords::NTuple{N, UInt32}, dir::Int, dims::NTuple{N, Int}) where N
    offs = lottery_offsets(topo)[dir]
    new_coords = ntuple(Val(N)) do i
        Base.@_inline_meta
        c = Int32(coords[i])
...
@inline function _local_site_update!(coords, grid, grid_dims, topology, cell_data, penalties, trackers, sampler, T_val, active_fraction, rng_state)
    ...
    if src_val != target_val
        ...
        ctx = (; grid=grid, grid_dims=grid_dims, topology=topology, cell_data=cell_data, trackers=trackers, idx=UInt32(idx), src=target_val, tgt=src_val, T=T_val, spatial_coords=coords, source_coords=source_coords, neighbors=neighbors, n_src=n_src, n_tgt=n_tgt)
        
        tx_deltas = evaluate_all_trackers(trackers, ctx)
        ctx_with_deltas = (; grid=ctx.grid, grid_dims=ctx.grid_dims, topology=ctx.topology, cell_data=ctx.cell_data, trackers=ctx.trackers, idx=ctx.idx, src=ctx.src, tgt=ctx.tgt, T=ctx.T, spatial_coords=ctx.spatial_coords, source_coords=ctx.source_coords, neighbors=ctx.neighbors, n_src=ctx.n_src, n_tgt=ctx.n_tgt, tx_deltas=tx_deltas)
...
@kernel function _local_lottery_sweep_kernel!(grid, grid_dims, topology, cell_data, penalties, trackers, sampler, T_val, active_fraction, global_seed)
    idx = @index(Global, Linear)
    
    if idx <= length(grid)
        my_ticket = pcg_hash(global_seed + UInt64(idx))
        
        prob_active = typeof(active_fraction)(my_ticket & 0x00FFFFFF) / typeof(active_fraction)(0x01000000)
        ...
        for dir in 1:N_moore
            ...
            if n_idx != idx
                ...
                if neighbor_ticket > my_ticket || (neighbor_ticket == my_ticket && n_idx > idx)
                    won_lottery = false
                    break
                end
            end
        ...
        if won_lottery
            ...
            _local_site_update!(coords, grid, grid_dims, topology, cell_data, penalties, trackers, sampler, T_val, active_fraction, rng_state)
        end
        end
    end
end

@kernel function _checkerboard_sweep_kernel!(grid, grid_dims, topology, cell_data, penalties, trackers, sampler, T_val, active_fraction, global_seed, color_indices, color_offset, num_active_pixels)
...
"""
    execute_step!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache, alg::CheckerboardMetropolis)

Executes a single Monte Carlo Sweep (MCS) across the grid using a deterministic checkerboard algorithm.
"""
function execute_step!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache, alg::CheckerboardMetropolis)
    T = alg.T
    active_fraction = alg.active_fraction
    sampler = alg.sampler
    
    @argcheck T >= zero(typeof(T)) "Temperature (T) must be non-negative"
    @argcheck zero(typeof(active_fraction)) <= active_fraction <= one(typeof(active_fraction)) "active_fraction must be between 0.0 and 1.0"
...
"""
    execute_step!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache, alg::ParallelMetropolis)

Executes a single Monte Carlo Sweep (MCS) across the grid using the stochastic lottery algorithm.
"""
function execute_step!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache, alg::ParallelMetropolis)
    T = alg.T
    active_fraction = alg.active_fraction
    sampler = alg.sampler
...
"""
    execute_step!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache, alg::SequentialMetropolis)

Executes a single Monte Carlo Sweep (MCS) sequentially on the CPU without atomic locks.
"""
function execute_step!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache, alg::SequentialMetropolis)
    T = alg.T
    active_fraction = alg.active_fraction
    sampler = alg.sampler
