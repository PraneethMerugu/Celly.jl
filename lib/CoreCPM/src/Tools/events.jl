...
"""
function process_mitosis_events!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache,
...
function reset_hst_fields_after_division!(u::AbstractCPMState, cache::CPMCache,
...
function update_step_auxiliary!(pen::AbstractHSTPenalty{FlexType}, u::AbstractCPMState,
        p::CPMParameters, cache::CPMCache, T_val, dt) where {FlexType}
    state_val = hst_state_field(pen)
    val_val = hst_value_field(pen)
    tgt_val = hst_target_field(pen)
    lam_val = lambda_field(pen)
