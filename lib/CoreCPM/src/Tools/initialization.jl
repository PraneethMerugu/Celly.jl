...
`:pressures`, etc. 
"""
function build_cell_data(grid::AbstractArray, N_cells::Int; FloatType=Float32, IntType=Int32, custom_fields...)
...
"""
    reallocate_cell_data!(u::AbstractCPMState, new_capacity::Int)

Dynamically grows the arrays inside `u.cell_data` to accommodate more cells 
(up to `new_capacity`), preserving existing data.
"""
function reallocate_cell_data!(u::AbstractCPMState, new_capacity::Int)
    ...
    # Users will need to manually do u = CPMState(u.grid, new_cell_data, ...) if reallocating from top-level
    error("reallocate_cell_data! is deprecated for immutable CPMState. Preallocate safely or re-construct.")
end
...
"""
function spawn_hypersphere!(grid, grid_dims, center::NTuple{N, Int}, radius::Int, cell_id::UInt32) where N
    r2 = radius^2
...
"""
    sync_cell_data!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache, num_cells::Int; set_targets::Bool=true)
...
"""
function sync_cell_data!(u::AbstractCPMState, p::CPMParameters, cache::CPMCache, num_cells::Int; set_targets::Bool=true)
    ...
    if set_targets && num_cells > 0
        ...
        if hasproperty(u.cell_data, :target_surface_areas) && hasproperty(u.cell_data, :surface_areas)
            copyto!(u.cell_data.target_surface_areas, 1, u.cell_data.surface_areas, 1, num_cells)
        end
    end
