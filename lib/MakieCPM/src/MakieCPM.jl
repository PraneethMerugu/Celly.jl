...
@recipe(CPMPlot, state) do scene
    Attributes(
        ...
        type_colors = ["#1C1C1E", "#FF3B30", "#007AFF", "#34C759", "#FF9500", "#AF52DE", "#5E5CE6", "#FF2D55"],
        slice = 0,
...
function Makie.plot!(plot::CPMPlot{<:Tuple{<:Any}})
    ...
    color_map_obs = lift(state_obs, plot.type_colors, plot.slice, plot.draw_boundaries, plot.boundary_color) do u, colors, slice_idx, draw_b, b_color
        cmap = Makie.to_colormap(colors)
        b_c = Makie.to_color(b_color)
        N_col = length(cmap)
        
        g_full = u.grid
        g = ndims(g_full) == 3 ? g_full[:, :, slice_idx <= 0 ? size(g_full, 3) ÷ 2 : slice_idx] : g_full
...
function explore_cpm(prob, alg; 
    ...
    if !isempty(metrics)
        ...
        for (i, (m_name, m_func)) in enumerate(metrics)
            ...
            y_vals_obs = lift(sol_obs) do s
                return Float32[Float32(applicable(m_func, s, i) ? m_func(s, i) : m_func(s.u[i])) for i in 1:length(s.t)]
            end
    ...
    if !isempty(parameters)
        ...
        for (p_name, p_spec) in parameters
            push!(sg_args, (label=p_name, range=p_spec.range, startvalue=p_spec.start))
        end
        ...
        on(recompute_btn.clicks) do _
            ...
            for (i, (p_name, p_spec)) in enumerate(parameters)
                ...
                elseif res !== nothing
                    if res isa SciMLBase.AbstractDiscreteProblem || res isa CoreCPM.AbstractCPMProblem
                        current_prob = res
                    else
                        current_alg = res
                    end
