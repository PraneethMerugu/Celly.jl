# MakieCPM Interactive Dashboard Showcase
...
function run_showcase()
        ...
        "Temperature (T)" => (
            ...
            action=(prob, alg, val) -> begin
                if alg isa ParallelMetropolis
                    return ParallelMetropolis(sweeps_per_step=alg.sweeps_per_step, T=typeof(alg.T)(val), active_fraction=alg.active_fraction, sampler=alg.sampler)
                else
                    return SequentialMetropolis(sweeps_per_step=alg.sweeps_per_step, T=typeof(alg.T)(val), active_fraction=alg.active_fraction, sampler=alg.sampler)
                end
            end
        ),
    ...
    my_metrics = [
        "Mean Volume" => u -> sum(@view u.cell_data.volumes[2:end]) / max(1, count(>(0), @view u.cell_data.volumes[2:end])),
