...
mutable struct BulkShearPenalty{T} <: CoreCPM.AbstractPenalty
    shear_rate::T
end
...
# ------------------------------------------------------------------
function brick_wall_init!(grid::Matrix{Int32}, n_cols::Int, n_rows::Int, dims::NTuple{2, Int})
    w, h = dims
    cell_w = w / n_cols
    cell_h = h / n_rows
...
dummy_cache = CoreCPM.CPMCache(prob.u0, prob.p.topology, 128)
CoreCPM.sync_cell_data!(prob.u0, prob.p, dummy_cache, 160; set_targets=true)
    ...
    fig = explore_cpm(
        ...
        parameters = [
            ...
            "Temperature" => (
                ...
                start = 1.5f0,
                action = (p, a, val) -> begin
                    return CoreCPM.SequentialMetropolis(
                        sampler = a.sampler,
