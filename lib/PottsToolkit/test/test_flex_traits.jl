...
@testset "Flex Traits Tests" begin
    ...
    @test prob.p.penalties[2] isa CorePotts.AdhesionPenalty{CorePotts.Flex}
    ...
    mod_src = CorePotts.get_adhesion_modifier(adh_pen, ctx, ctx.src)
    mod_tgt = CorePotts.get_adhesion_modifier(adh_pen, ctx, ctx.tgt)
