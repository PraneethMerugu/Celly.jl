...
Implementation maturity: Phases 5, 6, and 7 complete on the current CPU/Metal/ROCm backend contract
...
| [Auxiliary Constraints and Mechanical State](auxiliary-state-semantics.md) | exact scalar OU transition, mechanical-work fixtures, initialization laws, semantic RNG, and normalized-clock validators | Stable pressure/tension laws derived; historical real-HST claim rejected | `FluctuatingVolumePressure` and `FluctuatingSurfaceTension` pass CPU/Metal/ROCm sequential/checkerboard/lottery 2D/3D clocks, replay, initialization/evolution moments, and zero internal sync; full CorePotts suite: 2628 pass, 1 broken | Phase 8 lifecycle distributions; PottsToolkit migration owns legacy HST deletion |
