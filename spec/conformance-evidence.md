...
| [Transition-Kernel Verification](transition-kernel-verification.md) | `integration/transition/TransitionKernelOracle.jl`; `design/evidence/phase-13/exact/index.toml`; empirical CPU and Metal indices; registered CPU/Metal/ROCm realistic records and analyses | Independent primitive/sub-round/MCS oracles complete; sequential declared-law conformance and checkerboard non-equivalence are established; CPU and Metal v2 transition rows pass; the full applicable realistic battery is archived | Admit real-ROCm v2 transition evidence, then record the final guarantee labels and owner approval |
