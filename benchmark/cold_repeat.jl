...
backend in ("cpu", "metal", "amdgpu") || error(
    "Phase 12 cold repetitions require cpu, metal, or amdgpu")
repetitions in (1, 3, 5) || error("Phase 12 repetitions must be 1, 3, or 5")

worker = joinpath(@__DIR__, "cold_worker.jl")
project = Base.active_project()
run_id = get(ENV, "GITHUB_RUN_ID", "local")
run_attempt = get(ENV, "GITHUB_RUN_ATTEMPT", "1")
algorithms = ("SequentialCPM", "SequentialEquilibrium", "CheckerboardSweepCPM", "LotteryCPM")

for algorithm in algorithms, repetition in 1:repetitions
    process_id = "$run_id-$run_attempt-$backend-cold-$algorithm-$repetition"
    command = `$(Base.julia_cmd()) --project=$project --startup-file=no $worker --backend=$backend --algorithm=$algorithm`
