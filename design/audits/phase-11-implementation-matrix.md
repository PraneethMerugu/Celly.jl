...
Status: Complete; exact-head protected CPU/Metal/ROCm CI pass
...
| Draws | `draw`, `RandomDraw`, stable labels | unique semantic addresses, duplicates rejected, no authoring-time RNG | Bernoulli/Uniform/Normal/2D-3D unit-vector device lowering implemented; duplicate labels, literal domains, and compiled-operation collisions rejected; one permanent CPU/Metal/ROCm 2D/3D gate executes all four families |
| Queries | typed relations and lazy `neighbors`, `contacts`, `sites`, `boundary_sites` | result-domain, empty-policy, reduction and backend fixtures | typed `Contacting`, `AnyFiniteCell`, and `CellTypeFilter`; exact edge-count/contact-measure/boundary-site and distinct-neighbor count/property-sum/property-mean reductions implemented and oracle-checked on CPU/Metal/ROCm in 2D/3D; property reachability and explicit empty-mean policy are validated; materialized lazy collections are deferred until a required model justifies their storage and performance contract |
...
| CPU, Metal, and ROCm complete paths | CI jobs plus backend qualification scripts | permanent Phase 11 rule/query qualification is green on CPU, real Metal, and real ROCm in 2D/3D with zero warm-step sync/transfer/allocation; exact-head workflow `29803609678` uploaded both GPU evidence artifacts |
| Custom physics remains first class | direct Level 3 component and downstream Level 1 constructor fixtures | a zero-Core-edit downstream energy implements public CorePotts methods, is returned by an ordinary Julia constructor, composes directly into the high-level model, and is permanently qualified on CPU/Metal/ROCm in 2D/3D; no mutable registry or compiler switch exists |
