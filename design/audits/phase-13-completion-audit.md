...
Status: Evidence complete; owner freeze approval and post-approval final validation pending
...
| 13G | API audit, quality gates, legacy removal, frozen identities, owner approval | exhaustive API inventory; package/conformance/docs/install checks; current Metal AIR/trace and ROCm GCN/Perfetto archives; legacy closure; owner packet | candidate proved; final status changes and owner approval pending |
...
| Deterministic, moderate statistical, and scheduled/pre-release tiers | 2,465-test CPU conformance shard plus CPU/Metal/ROCm empirical and realistic workflows | complete for the registered evidence |
| Specification-to-test evidence index | `spec/conformance-evidence.md` | evidence complete; final metadata pending |
...
Phase 13 must not be marked complete until item 8 is closed, the final public metadata matches the
admitted evidence, and the complete validation matrix passes on that exact final source.
