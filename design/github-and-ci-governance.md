...
- Required checks run on GitHub-hosted infrastructure and use only read access to repository
  contents.
...
- Self-hosted runners never execute code from fork pull requests. GPU jobs are eligible only for
  pushes, manual dispatches, and pull requests whose head repository is this repository.
...
- Hosted CPU CI is the always-available correctness gate.
