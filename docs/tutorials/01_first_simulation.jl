...
# # First Simulation
#
# Welcome to the Celly.jl tutorials. This first tutorial walks through
# every step of a complete Cellular Potts Model (CPM) simulation — from
# defining cell types to recording a movie — so you have a working mental
# model before diving into more specialised topics.

# ## Packages
#
# `CPMToolkit` is the single entry point for users. It re-exports the physics
# engine (`CoreCPM`) automatically, so you never need to import `CoreCPM`
# directly. `MakieCPM` adds visualisation and recording utilities.
...
# ## Defining Cell Types
#
# A CPM universe is partitioned into named cell types. Every lattice site
...
#
# The CPM Hamiltonian is a sum of penalty terms. Here we use two:
#
...
#
# `CPMProblem` binds a system to an initial cell count, a grid geometry, and
...
# ## Solving
#
# `solve` runs the simulation and returns a `CPMSolution`. `saveat = 5` saves
...
#
# `record_cpm` writes an MP4 using every saved snapshot as a frame. The
