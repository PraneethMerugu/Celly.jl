# [MakieCPM](@id makiecpm-overview)

**MakieCPM** provides visualization tools for CPM simulations built on top of
[Makie.jl](https://docs.makie.org).
It offers three main entry points:

| Function | Purpose |
|----------|---------|
| `cpmplot` / `cpmplot!` | Static plot of a single simulation state |
| `explore_cpm` | Interactive dashboard with live re-simulation |
| `record_cpm` | Render a simulation solution to a video file |
...
> [!NOTE]
> MakieCPM uses Makie's backend-agnostic recipe system. Load a Makie backend
> (`using GLMakie`, `using CairoMakie`, or `using WGLMakie`) before calling MakieCPM
> functions. For interactive features (`explore_cpm`) you need `GLMakie` or `WGLMakie`.

---

## `cpmplot` — Static State Plot

`cpmplot` is a full Makie plot recipe that renders a single CPM lattice state as a
colour-coded image, where each cell type is assigned a distinct colour.

```julia
using GLMakie, MakieCPM

# Plot the final state of a solution
state = sol[end]
fig, ax, p = cpmplot(state)
...
`cpmplot!` is the mutating variant for composing into an existing `Axis`:
...
## `record_cpm` — Video Recording

`record_cpm` iterates over all saved frames of a `solve` solution and renders them to
a video file (MP4, GIF, or any format supported by Makie's `record`).

```julia
record_cpm("output.mp4", sol;
    framerate  = 30,
    resolution = (1400, 900),
    metrics    = [
...
```julia
using GLMakie, MakieCPM, Statistics
