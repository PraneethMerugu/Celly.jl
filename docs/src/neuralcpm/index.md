# [NeuralCPM](@id neuralcpm-overview)

**NeuralCPM** extends the CPM framework by replacing hand-crafted penalty functions with
...
simulation trajectories (or microscopy data) and NeuralCPM trains a Lux.jl neural network
...
## Energy-Based Models in the CPM Context
...
In the CPM context, $\mathbf{x}$ is a lattice configuration $\sigma$ and
...
for large lattices, which is why NeuralCPM uses **Contrastive Divergence** training
...
and plugs it into the CoreCPM penalty system.
...
NeuralCPM trains the network via **Persistent Contrastive Divergence** (PCD), a
...
`CPMTrainingCache` initialises `n_persistent_chains` independent simulation states that
will serve as the *negative phase* samples. They are updated by running the CPM forward
...
for epoch in 1:1000
    loss, grads = Zygote.withgradient(θ) do θ
        cpm_loss(cache, positive_states, network, θ, state)
    end
...
All NeuralCPM operations are compatible with [Zygote.jl](https://fluxml.ai/Zygote.jl/),
Julia's source-to-source automatic differentiation engine.

The gradient $\nabla_\theta \mathcal{L}$ is computed by differentiating through the
neural network calls inside `cpm_loss`. The CPM simulation steps themselves are treated
as *black boxes* for the negative phase (no backprop through the MCMC chain), which is
the standard PCD approximation.

> [!TIP]
> Because the CPM kernel calls inside `cpm_loss` are not differentiated, you can use
...
```julia
using CPMToolkit, NeuralCPM, Lux, Zygote, Optimisers
...
sys  = CPMSystem([A, Medium], [penalty])
prob = CPMProblem(sys, Dict(A => 30), (100, 100); tspan=(0, 200))
alg  = CheckerboardMetropolis(T=1.0f0, sweeps_per_step=5)

# 2. Prepare training cache and data
cache           = CPMTrainingCache(prob, alg; n_persistent_chains=4, saveat=5)
...
for epoch in 1:500
    loss, grads = Zygote.withgradient(
        θ -> cpm_loss(cache, positive_states, network, θ, st), θ)
    opt_state, θ = Optimisers.update(opt_state, θ, grads[1])
end
...
- [NeuralCPM API Reference](@ref neuralcpm-api) — Full docstring index.
