# [Backends](@id corecpm-backends)

CoreCPM separates the *simulation* from *output storage*.
...
- Compatible with all MakieCPM functions (`cpmplot`, `record_cpm`, `explore_cpm`).
...
`CoreCPM.CoreCPMBase.AbstractBackend` and implementing the `write_frame!` method:

```julia
import CoreCPM.CoreCPMBase: AbstractBackend, write_frame!
