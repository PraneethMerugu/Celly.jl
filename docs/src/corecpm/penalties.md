# [Penalties](@id corecpm-penalties)

Penalty terms are the building blocks of the CPM Hamiltonian.
...
In CPMToolkit you add penalties to a system via **Components** (e.g. `VolumeComponent`,
`AdhesionComponent`). Each component internally constructs the corresponding CoreCPM
...
training with `NeuralCPM`. See [Concepts → HST Penalties](@ref concepts) for the
...
| `HSTVolumeComponent` | ✓ | HST OU | Dynamic targets / NeuralCPM |
